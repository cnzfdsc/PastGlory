...
public class skInput : MonoBehaviour
{
	...
	{
		if (m_InputTouchs[0].FingerID >= 0 && m_InputTouchs[0].Phase == TouchPhase.Moved)
		{
	...
	{
		if (m_InputTouchs[0].FingerID >= 0 && m_InputTouchs[0].Phase == TouchPhase.Ended)
		{
			touch = m_InputTouchs[0];
			return true;
		}
	...
	{
		if (m_InputTouchs[0].FingerID >= 0 && m_InputTouchs[0].Click)
		{
			touch = m_InputTouchs[0];
			return true;
		}
	...
	{
		if (m_InputTouchs[1].FingerID >= 0 && m_InputTouchs[1].Click)
		{
	...
	{
		if (m_InputTouchs[0].FingerID >= 0 && m_InputTouchs[0].TapCount == 2)
		{
			touch = m_InputTouchs[0];
			return true;
		}
	...
	protected void Update()
	{
		...
		if (Input.GetMouseButton(0)) // 鼠标左键
		{
			...
			touch0.FingerID = 0;
			touch0.Click = false;
			// Windows上不触发TouchPhase.Canceled状态. 鼠标按键松开就是TouchPhase.Ended
			// Windows上不触发TouchPhase.stationary状态. 不记得有什么操作需要鼠标在同一个地方按下不动的.
			touch0.Phase = touch0.Phase == TouchPhase.Ended
								? TouchPhase.Began
								: TouchPhase.Moved;
		...
		if (Input.GetMouseButton(1)) // 鼠标右键
		{
			...
			touch1.FingerID = 1;
			touch1.Click = false;
			touch1.Phase = touch1.Phase == TouchPhase.Ended
								? TouchPhase.Began
								: TouchPhase.Moved;
	...
	#region Public变量
	[SerializeField]
	[Tooltip("按下鼠标按键和松开鼠标按键之间, 最长间隔多久会被认为是一次单击")]
	private float MAX_CLICK_TIME = 0.1f;
	[SerializeField]
	[Tooltip("两次松开鼠标按键之间, 最长间隔多久会被认为是一次双击")]
	private float MAX_DOUBLE_CLICK_TIME = 0.2f;
	...
	{
		/// <summary>
		/// 不光FingerID表示触摸的ID, 还用来表示当前InputTouch的数据是否可用.
